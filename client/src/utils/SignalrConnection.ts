...
class Connector {
    ...
    public events: (
        ...
        onOperationTransformedAck: (newRevisionId: string) => void
    ) => void;
    ...
    public newOperation = (op: Operation, documentId: string) => {
        this.connection.send("newOperation", op, documentId).then(x => console.log("newOperation sent"))
    }
    public addToGroup = (groupName: string) => {
        this.connection.send("addToGroup", groupName).then(x => console.log("AddToGroup sent"))
    }
